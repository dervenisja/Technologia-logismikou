...
public class PartyController {
    ...
    public void processSubscription(PartyHost host, String plan) {
        System.out.println("⚙️ [PartyController] Επεξεργασία αναβάθμισης συνδρομής...");

        // 1. Ενημέρωση στη μνήμη (Entity)
        host.setSubscriptionLevel(plan);

        // 2. Ενημέρωση στη Βάση Δεδομένων (Persistence)
        PersistenceController.updateHostSubscription(host.getHostId(), plan);

        System.out.println("✅ [PartyController] Η συνδρομή ενημερώθηκε επιτυχώς σε " + plan);
    }
